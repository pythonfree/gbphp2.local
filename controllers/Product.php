...
if (isset($_GET['addlimit'])) {
    $addLimit =  htmlspecialchars($_GET['addlimit']);
    $_SESSION['limit'] += (int)$addLimit;
} else {
    $_SESSION['limit'] = 4;
}

$allGoodsCount = allGoodsCount($connect, 'goods');
if ($_SESSION['limit'] > $allGoodsCount) {
    $_SESSION['limit'] = $allGoodsCount;
}
$goods = getAllByLimit($connect, 'goods', $_SESSION['limit']);
